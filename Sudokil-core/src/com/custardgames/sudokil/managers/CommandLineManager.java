...
public class CommandLineManager implements EventListener
{
	...
	public ItemCLI findItem(String location)
	{
		...
		for (int x = 0; x < locations.length; x++)
		{
			if (locations[x].length() > 0)
			{
				...
				{
					if (!newItem.getParent().equals("") && newItem.getParent() != null)
					{
				...
				{
					for (String e : ((FolderCLI) newItem).getItems())
					{
						ItemCLI item = root.getItem(e);
						if (item instanceof FolderCLI)
						{
							if (locations[x].equals(item.getName()))
							{
								newItem = (FolderCLI) item;
								foundLocation = true;
								break;
							}
						}
						else if (item instanceof ItemCLI)
						{
							if (locations[x].equals(item.getName()))
							{
								newItem = item;
	...
	{
		String output = "";
		for (String e : currentItem.getItems())
		{
			output += root.getItem(e).getName() + "\n";
		}
	...
	public Array<String> autoComplete(String text)
	{
		...
		for (String e : ((FolderCLI) tempLocation).getItems())
		{
			if (root.getItem(e).getName().startsWith(locations[locations.length - 1]))
			{
				String result = root.getItem(e).getName();
